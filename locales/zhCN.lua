
local addon = TinyTooltip or select(2, ...)

if (GetLocale() ~= "zhCN") then return end

addon.L = addon.L or {}
local L = addon.L
local T = {
    ["about.announcement.chat"] = "暴雪已修复导致物品和法术提示出现Lua报错的问题，同时在综合设置页面也添加了一个选项来控制是否显示最新公告，您可在设置页面查看完整信息。",
    ["about.announcement.chatKey"] = "announcement_2026_03_31_tooltip_bug",
    ["about.announcement.content"] = "根据测试，暴雪已经解决了物品和法术提示会变成秘密值而导致勾选显示物品/法术图标时会出现大量错误的问题，您可以重新开启这2个选项。"
    .."此外，1.6.2版本中在综合页面新增了一个选项用以控制登录时是否显示公告信息。\n"
    .."目前确认到的问题有副本中进入战斗后会导致玩家的鼠标提示尺寸错误，这是赛季开始后新出现的问题，同时尺寸错误的问题仅仅随机出现在所查看的部分玩家身上，"
    .."因此我有理由怀疑这是由于暴雪方面所导致并非本插件引起，同时原TinyTooltip的架构设计导致了针对尺寸计算和内容的插入有多个入口并且鼠标提示会在多个不同位置重建，"
    .."这种架构设计使得针对鼠标提示尺寸异常的问题修复起来极为困难，尤其是当目前甚至还不能确定这种尺寸错误是否是由于插件导致还是由于暴雪本身导致。"
    .."基于以上原因，针对尺寸异常的问题暂时不会推送更新，并且我计划在未来1-2个功能性更新后彻底重构鼠标提示的创建流程，让整个创建管线保持单一的尺寸计算入口和调用单一的内容插入接口"
    .."以彻底解决尺寸计算复杂和创建过程中需要多次重绘鼠标提示的问题。\n"
    .."\n最后，如果在副本战斗后出现了玩家身上鼠标提示尺寸异常的错误请重载界面来解决。再次感谢您的理解与支持。",
    ["about.announcement.title"] = "公告",
    ["about.author.label"] = "作者",
    ["about.author.name"] = "星野绫 - 罗宁 CN",
    ["about.credits.content"] = "感谢TinyTooltip原作者 M 及所有贡献者\n",
    ["about.credits.title"] = "致谢",
    ["about.desc"] = "强大的鼠标提示工具, 基于TinyTooltip",
    ["about.help.title"] = "提交 Bug / 反馈",
    ["about.help.url"] = "https://github.com/nc-hyw/TinyTooltip-Remake/issues",
    ["about.version.label"] = "版本",

    ["anchor.none"] = "无",
    ["anchor.offset.locked"] = "锚定位置为下方时无法调节偏移量",

    ["axis.x"] = "X",
    ["axis.y"] = "Y",

    ["button.diySettings"] = "自定义设置",
    ["button.export"] = "导出",
    ["button.import"] = "导入",
    ["button.resetAll"] = "恢复全部设置",
    ["button.resetSection"] = "恢复默认设置",
    ["button.save"] = "保存",

    ["fontFlag.default"] = "系统默认",
    ["fontFlag.NORMAL"] = "无描边",
    ["fontFlag.OUTLINE"] = "描边",
    ["fontFlag.THINOUTLINE"] = "细描边",

    ["dropdown.Interface\\AddOns\\TinyTooltip-Remake\\texture\\StatusBar"] = "默认状态条",
    ["dropdown.alpha"] = "透明",
    ["dropdown.alt"] = "Alt",
    ["dropdown.angular"] = "直角边框",
    ["dropdown.auto"] = "智能匹配",
    ["dropdown.bottom"] = "底部",
    ["dropdown.class"] = "职业染色",
    ["dropdown.ctrl"] = "Ctrl",
    ["dropdown.cursor"] = "|cff33ccff鼠标|r",
    ["dropdown.cursorRight"] = "鼠标右侧",
    ["dropdown.dark"] = "深黑",
    ["dropdown.default"] = "|cffaaaaaa系统默认|r",
    ["dropdown.faction"] = "阵营染色",
    ["dropdown.global"] = "全局设置",
    ["dropdown.gradual"] = "渐变",
    ["dropdown.inarena"] = "竞技场",
    ["dropdown.incombat"] = "战斗中",
    ["dropdown.inherit"] = "|cffffee00继承全局|r",
    ["dropdown.ininstance"] = "副本时",
    ["dropdown.inpvp"] = "战场里",
    ["dropdown.inraid"] = "团队时",
    ["dropdown.itemLevel"] = "物品等级(暴雪)",
    ["dropdown.itemQuality"] = "物品品质染色",
    ["dropdown.level"] = "等差染色",
    ["dropdown.marble"] = "大理石",
    ["dropdown.none"] = "|cffaaaaaa无|r",
    ["dropdown.not inarena"] = "|cffff3333非|r竞技场",
    ["dropdown.not incombat"] = "|cffff3333非|r战斗中",
    ["dropdown.not ininstance"] = "|cffff3333非|r副本时",
    ["dropdown.not inpvp"] = "|cffff3333非|r战场里",
    ["dropdown.not inraid"] = "|cffff3333非|r团队时",
    ["dropdown.not reaction5"] = "|cffff3333非|r声望友好以上",
    ["dropdown.not reaction6"] = "|cffff3333非|r声望尊敬以上",
    ["dropdown.not samecrossrealm"] = "|cffff3333非|r同跨服务器",
    ["dropdown.not samerealm"] = "|cffff3333非|r同服务器",
    ["dropdown.reaction"] = "声望染色",
    ["dropdown.reaction5"] = "声望友好以上",
    ["dropdown.reaction6"] = "声望尊敬以上",
    ["dropdown.rock"] = "岩石",
    ["dropdown.samecrossrealm"] = "同跨服务器",
    ["dropdown.samerealm"] = "同服务器",
    ["dropdown.selection"] = "类型染色",
    ["dropdown.shift"] = "Shift",
    ["dropdown.smooth"] = "百分比动态",
    ["dropdown.static"] = "|cff33ccff固定位置|r",
    ["dropdown.top"] = "顶部",

    ["general.bodyFont"] = "内容字体",
    ["general.bodyFontFlag"] = "内容字体描边",
    ["general.bodyFontSize"] = "内容字体大小",
    ["general.headerFont"] = "标头字体",
    ["general.headerFontFlag"] = "标头字体描边",
    ["general.headerFontSize"] = "标头字体大小",

    ["general.anchor"] = "锚点器",
    ["general.anchor.hiddenInCombat"] = "战斗时隐藏",
    ["general.anchor.modifierShowInCombat"] = "隐藏时按住修饰键显示",
    ["general.anchor.modifierShowInCombatKey"] = "修饰键",
    ["general.anchor.position"] = "框架锚点",
    ["general.anchor.returnInCombat"] = "战斗时固定至默认位置",
    ["general.anchor.returnOnUnitFrame"] = "单位框体时固定至默认位置",
    ["general.announcementMode"] = "公告信息",
    ["general.announcements"] = "公告信息",
    ["general.announcements.dropdown.noticeAlways"] = "始终显示公告信息",
    ["general.announcements.dropdown.noticeNever"] = "从不显示公告信息",
    ["general.announcements.dropdown.noticeSnooze"] = "每次公告更新时仅显示一次",
    ["general.annoucements"] = "公告信息",
    ["general.annoucements.dropdown.noticeAlways"] = "始终显示公告信息",
    ["general.annoucements.dropdown.noticeNever"] = "从不显示公告信息",
    ["general.annoucements.dropdown.noticeSnooze"] = "每次公告更新时仅显示一次",
    ["general.background"] = "背景颜色",
    ["general.bgfile"] = "背景材质",
    ["general.border.none"] = "无边框",
    ["general.borderColor"] = "边框颜色",
    ["general.borderCorner"] = "边框样式",
    ["general.borderSize"] = "边框大小",
    ["general.hideUnitFrameHint"] = "隐藏头像框右键设置提示",
    ["general.mask"] = "顶部遮罩",
    ["general.quickFocusModKey"] = "快速焦点修饰键",
    ["general.SavedVariablesPerCharacter"] = "为每个角色保存独立配置",
    ["general.scale"] = "框架缩放",
    ["general.skinMoreFrames"] = "样式应用于更多框架 |cffcccc33(重载生效)|r",
    ["general.statusbarColor"] = "HP颜色",
    ["general.statusbarFont"] = "HP文字字体",
    ["general.statusbarFontFlag"] = "HP字体描边",
    ["general.statusbarFontSize"] = "HP文字大小",
    ["general.statusbarHeight"] = "HP高度",
    ["general.statusbarHide"] = "隐藏状态条",
    ["general.statusbarOffsetX"] = "HP位置X边距",
    ["general.statusbarOffsetY"] = "HP位置Y偏移",
    ["general.statusbarPercent"] = "显示HP百分比",
    ["general.statusbarPosition"] = "HP位置",
    ["general.statusbarText"] = "显示HP数字",
    ["general.statusbarTexture"] = "HP背景材质",

    ["hint.anchor.returnInCombat"] = "当在战斗中获取鼠标提示时将显示在默认位置",
    ["hint.anchor.returnOnUnitFrame"] = "单位框体获取的鼠标提示将被固定在默认位置",

    ["id.bonus"] = "附加属性ID",
    ["id.enhancement"] = "附魔ID",
    ["id.expansion"] = "资料片",
    ["id.gem"] = "宝石ID",
    ["id.icon"] = "图标ID",
    ["id.item"] = "物品ID",
    ["id.maxStack"] = "最大堆叠数",
    ["id.na"] = "N/A",
    ["id.quest"] = "任务ID",
    ["id.spell"] = "法术ID",

    ["item.coloredItemBorder"] = "物品边框染色",
    ["item.modifierShowAll"] = "按住任意修饰键显示所有信息",
    ["item.showItemBonusId"] = "显示附加属性ID",
    ["item.showItemEnhancementId"] = "显示附魔ID",
    ["item.showItemExpansion"] = "显示资料片",
    ["item.showItemGemId"] = "显示宝石ID",
    ["item.showItemIcon"] = "显示物品图标",
    ["item.showItemIconId"] = "显示物品图标ID",
    ["item.showItemId"] = "显示物品ID",
    ["item.showItemMaxStack"] = "显示最大堆叠数",

    ["<Drag element to customize the style>"] = "<拖动元素以自定义样式>",
    ["Achievement"] = "成就",
    ["Anchor"] = "锚点器",
    ["DIY"] = "自定义",
    ["collected"] = "已收集",
    ["ItemLevel"] = "物品等级",
    ["Mount"] = "坐骑",
    ["mount"] = "坐骑",
    ["Mythic+ Score"] = "史诗钥石评分",
    ["showTargetBy"] = "显示被关注",
    ["TargetBy"] = "被关注",
    ["uncollected"] = "未收集",
    ["unknown"] = "??",

    ["menu.font"] = "字体",
    ["menu.general"] = "综合",
    ["menu.item"] = "物品",
    ["menu.npc"] = "NPC",
    ["menu.player"] = "玩家",
    ["menu.spell"] = "法术",
    ["menu.statusbar"] = "状态条",
    ["menu.variables"] = "导入/导出",

    ["popup.reloadNotice.text"] = "提示示例：ipsum loream",

    ["quest.coloredQuestBorder"] = "任务边框染色",
    ["quest.showQuestId"] = "显示任务ID",

    ["quickfocus.help"] = "按住修饰键并点击目标以设置焦点，按住修饰键并点击空白处以取消焦点。(点击单位框架时该功能无效)",

    ["spell.background"] = "背景颜色",
    ["spell.borderColor"] = "边框颜色",
    ["spell.modifierShowAll"] = "按住任意修饰键显示所有信息",
    ["spell.showIcon"] = "法术图标",
    ["spell.showSpellIconId"] = "显示法术图标ID",
    ["spell.showSpellId"] = "显示法术ID",

    ["unit.npc.anchor.hiddenInCombat"] = "战斗时隐藏",
    ["unit.npc.anchor"] = "锚点器",
    ["unit.npc.anchor.modifierShowInCombatKey"] = "修饰键",
    ["unit.npc.anchor.position"] = "框架锚点",
    ["unit.npc.anchor.returnInCombat"] = "战斗时固定至默认位置",
    ["unit.npc.anchor.returnOnUnitFrame"] = "单位框体时固定至默认位置",
    ["unit.npc.background.alpha"] = "透明度",
    ["unit.npc.background"] = "背景染色",
    ["unit.npc.coloredBorder"] = "边框染色",
    ["unit.npc.elements.classIcon"] = "职业图标",
    ["unit.npc.elements.classifBoss"] = "首领",
    ["unit.npc.elements.classifElite"] = "精英",
    ["unit.npc.elements.classifRare"] = "稀有",
    ["unit.npc.elements.creature"] = "类型",
    ["unit.npc.elements.factionBig"] = "右上阵营大图",
    ["unit.npc.elements.levelValue"] = "等级",
    ["unit.npc.elements.moveSpeed"] = "移动速度",
    ["unit.npc.elements.name"] = "名称",
    ["unit.npc.elements.npcTitle"] = "头衔",
    ["unit.npc.elements.questIcon"] = "任务图标",
    ["unit.npc.elements.raidIcon"] = "标记图标",
    ["unit.npc.elements.reactionName"] = "声望",
    ["unit.npc.grayForDead"] = "死亡目标灰度染色",
    ["unit.npc.showModel"] = "显示模型",
    ["unit.npc.showTarget"] = "显示目标",
    ["unit.npc.showTargetBy"] = "显示被关注数",

    ["unit.player.anchor.hiddenInCombat"] = "战斗时隐藏",
    ["unit.player.anchor"] = "锚点器",
    ["unit.player.anchor.modifierShowInCombatKey"] = "修饰键",
    ["unit.player.anchor.position"] = "框架锚点",
    ["unit.player.anchor.returnInCombat"] = "战斗时固定至默认位置",
    ["unit.player.anchor.returnOnUnitFrame"] = "单位框体时固定至默认位置",
    ["unit.player.background.alpha"] = "透明度",
    ["unit.player.background"] = "背景染色",
    ["unit.player.coloredBorder"] = "边框染色",
    ["unit.player.elements.achievementPoints"] = "成就点数",
    ["unit.player.elements.achievementPoints.icon"] = "使用成就图标",
    ["unit.player.elements.classIcon"] = "职业图标",
    ["unit.player.elements.className"] = "职业",
    ["unit.player.elements.className.icon"] = "使用专精图标",
    ["unit.player.elements.factionBig"] = "右上阵营大图",
    ["unit.player.elements.factionIcon"] = "阵营图标",
    ["unit.player.elements.factionName"] = "阵营",
    ["unit.player.elements.friendIcon"] = "好友图标",
    ["unit.player.elements.gender"] = "性别",
    ["unit.player.elements.guildIndex"] = "公会阶级数字",
    ["unit.player.elements.guildName"] = "公会名称",
    ["unit.player.elements.guildRank"] = "公会阶级名称",
    ["unit.player.elements.guildRealm"] = "公会服务器",
    ["unit.player.elements.icon"] = "图标",
    ["unit.player.elements.isPlayer"] = "玩家",
    ["unit.player.elements.itemLevel"] = "物品等级",
    ["unit.player.elements.itemLevel.icon"] = "使用物品等级图标",
    ["unit.player.elements.levelValue"] = "等级",
    ["unit.player.elements.mount"] = "坐骑",
    ["unit.player.elements.mount.icon"] = "使用坐骑图标",
    ["unit.player.elements.moveSpeed"] = "移动速度",
    ["unit.player.elements.mplusScore"] = "史诗钥石评分",
    ["unit.player.elements.mplusScore.icon"] = "使用钥石图标",
    ["unit.player.elements.name"] = "名称",
    ["unit.player.elements.pvpIcon"] = "PVP状态",
    ["unit.player.elements.raceName"] = "种族",
    ["unit.player.elements.raidIcon"] = "标记图标",
    ["unit.player.elements.realm"] = "服务器",
    ["unit.player.elements.role"] = "职责",
    ["unit.player.elements.roleIcon"] = "职责图标",
    ["unit.player.elements.statusAFK"] = "AFK",
    ["unit.player.elements.statusDC"] = "OFFLINE",
    ["unit.player.elements.statusDND"] = "DND",
    ["unit.player.elements.title"] = "头衔",
    ["unit.player.elements.zone"] = "地区",
    ["unit.player.grayForDead"] = "死亡目标灰度染色",
    ["unit.player.showModel"] = "显示模型",
    ["unit.player.showTarget"] = "显示目标",
    ["unit.player.showTargetBy"] = "显示被关注",

    ["wildcard.help"] = "自定义格式:按回车生效",
    ["wildcard.help.example"] = "示例:(%s) 或 [%s]",
    ["wildcard.help.moveSpeed"] = "示例:%d%%",
}
for k, v in pairs(T) do
    L[k] = v
end
